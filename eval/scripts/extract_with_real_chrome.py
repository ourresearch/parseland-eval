"""Pilot Pass C: extract metadata using the user's real Chrome profile via
agent-browser --auto-connect.

Same structure as extract_with_agent_browser.py but attaches to a Chrome
instance the user has launched with `--remote-debugging-port=9222` and their
Gmail-linked profile, so the session carries real cookies + real fingerprint.
Output paths are different so this does not clobber Pass A artefacts.

Prereq: before running this script, user must run (macOS):
    open -a "Google Chrome" --args --remote-debugging-port=9222 --profile-directory="Default"

Writes:
- `eval/data/random-50-chrome.csv` — gold-standard column order
- `eval/data/random-50-chrome.meta.json` — per-row cost, duration, snapshot path
- `eval/data/snapshots-chrome/{sha1(doi)}.json` — per-DOI page snapshots
"""
...
SNAPSHOT_DIR = EVAL_DIR / "data" / "snapshots-chrome"
...
def fetch_page(url: str) -> AgentBrowserResult:
    errors: list[str] = []
    open_res = _run_ab(["open", url, "--auto-connect"])
