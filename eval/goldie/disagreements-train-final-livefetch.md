# Disagreements

## DOI: 10.1016/j.celrep.2018.10.057

**pdf_url**
- AI:    http://www.cell.com/article/S2211124718316462/pdf
- Human: https://www.cell.com/action/showPdf?pii=S2211-1247%2818%2931646-2
- landing_page_truth: TBD

## DOI: 10.1016/j.clpl.2024.100067

**rases**
- AI:    [
  {
    "name": "Giulia Alessandri",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Tiberio Daddi",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Fabio Iraldo",
    "rasses": "",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Giulia Alessandri",
    "rasses": "Scuola Superiore Sant'Anna, Piazza Martiri della Libertà 33, Pisa, PI, 56127, Italy",
    "corresponding_author": true
  },
  {
    "name": "Tiberio Daddi",
    "rasses": "Scuola Superiore Sant'Anna, Piazza Martiri della Libertà 33, Pisa, PI, 56127, Italy",
    "corresponding_author": false
  },
  {
    "name": "Fabio Iraldo",
    "rasses": "Scuola Superiore Sant'Anna, Piazza Martiri della Libertà 33, Pisa, PI, 56127, Italy",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**corresponding**
- AI:    [
  {
    "name": "Giulia Alessandri",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Tiberio Daddi",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Fabio Iraldo",
    "rasses": "",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Giulia Alessandri",
    "rasses": "Scuola Superiore Sant'Anna, Piazza Martiri della Libertà 33, Pisa, PI, 56127, Italy",
    "corresponding_author": true
  },
  {
    "name": "Tiberio Daddi",
    "rasses": "Scuola Superiore Sant'Anna, Piazza Martiri della Libertà 33, Pisa, PI, 56127, Italy",
    "corresponding_author": false
  },
  {
    "name": "Fabio Iraldo",
    "rasses": "Scuola Superiore Sant'Anna, Piazza Martiri della Libertà 33, Pisa, PI, 56127, Italy",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**pdf_url**
- AI:    https://www.sciencedirect.com/science/article/pii/S2666791624000137/pdfft
- Human: https://pdf.sciencedirectassets.com/777795/1-s2.0-S2666791624X00020/1-s2.0-S2666791624000137/main.pdf?X-Amz-Security-Token=IQoJb3JpZ2luX2VjEAQaCXVzLWVhc3QtMSJIMEYCIQCpflXzdaraz0J9VrgP7%2FQLuN1hYRTT%2B6HJdvGpujRqsAIhAJr822Q0wx01mGMghgX%2FdtQxdVHhakG%2F%2FmPerIWTrYIFKrwFCMz%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FwEQBRoMMDU5MDAzNTQ2ODY1IgzThvszzXSYLbvCI48qkAVGmwGeWil5oZI0hT60URNJwFowCZHO8Aido%2BeCXxRU3uhjcls%2BOImj2EnKVFN0VAKQT9yiYpEq4EuJr0lYciz%2F5m8tWSTpjJDeM54J5my%2F3F3UqBQJau14pJQow8qoiKu1Ye%2BnY4vH7OkCJYFaE1%2B%2F%2Bu0ko6Aap1luAi6cOsYHRBc2NgrFMuaxsTEXp98uDxnIRRsu%2FHK9kM132m27uxv9oPNLGN7puxCvdbcNjtq3%2BGbY1F2%2FiEsdxnPFHfiWJF%2FFugUMlz57taD59svZ%2BC%2Fvdipt03SZYKUtQRD7d6HyPu7Tz0NAy8n918uLLmX7PAfMh1aACw8Ve2NdtechB2f0SDRp6ZlcLC0oLJ92C0MRTVFkQT2YNbX73DURNRTHH%2FNx9CRcSobhEvecXrI5t13ibdEMiZZopRdl%2FY%2FO%2FFX88tO%2BRndJ8mDL1Jh%2B5BBhSG9X%2FtbynvappM7DPbGgb5W2AgTob%2BEDKHk6OqJikc071%2Beo5TGfcW6zpcMYbkIxQ%2F8lG2mKIYTNenrvNdLcr9WQHO5BNFM%2Fm%2Bei%2FWBlih8A3mpty5Qsss3Rqn42EdmehU%2B3JhNKANjdeK8B%2B83%2FwYK7uck%2BD%2BIbE2rR%2FSu3n8pNPU06vz0%2Fm6d8%2Bn%2BFH98lf2aYI%2B7jH8pK50A%2FTCxlcXSwvBrPVKOPFTJn1bU0%2FQFp84actFRFjbcr9S5aeUuMcij5unUTx5MwlxQ5pEKogRWBsomkel57Crh6WEH%2BzTCTVYscizOvQdrZ404IZMqDUJEpQ2HlnHbXtsnw08BGneex%2BOcHsea7EXu7eAOhWCT4jZG2QOGnHC3LcXgXWkGgT4%2BmFuaTsJKFNjDWNyniNLtykfvFXOQlfQf0d6gd642XkDDn577PBjqwAXFO5Jq65zPmqaN%2F1BLKyYiYsTRiR%2B%2BjOFv%2BM96gI7aMXTo%2BgzXmzUEFQ0w8kBBJYvcRi7cBIO4c3ZH%2Fdbhrk6GEg3fKXbXWG5d1%2F3P4tJn3rUU2XXp8dSQZGihBKlUq462q306a%2FkoZOTAzJtA%2FM%2B7vc%2Fw2IpVHeayjt7AmD8C2u8DFJyH1hb47nOjgfbLkZIyjteMo%2FYk6ZiS19LLbI6yZ6y%2FyMDaYfsoWU8yTTXUM&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20260427T193130Z&X-Amz-SignedHeaders=host&X-Amz-Expires=299&X-Amz-Credential=ASIAQ3PHCVTYXEHLNRL7%2F20260427%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=c1f537e5c0c0dbc65c310de01a9a7503ac867b9faaff5399655a9a395f59e8bd&hash=43866301e66d3a15a440140a7bb0057c94bbb519abb91a0b5644b073cc57bd55&host=68042c943591013ac2b2430a89b270f6af2c76d8dfd086a07176afe7c76c2c61&pii=S2666791624000137&tid=spdf-dff3e078-cea1-4e17-aa38-c42bbf999cda&sid=036a10659e7bd4475e2b9df767e902b1bd23gxrqa&type=client&tsoh=d3d3LnNjaWVuY2VkaXJlY3QuY29t&rh=d3d3LnNjaWVuY2VkaXJlY3QuY29t&ua=0f15570357535d08575506&rr=9f303f75df0e16d1&cc=us
- landing_page_truth: TBD

## DOI: 10.1016/j.jallcom.2006.06.063

**authors**
- AI:    [
  {
    "name": "R.V. Wandekar",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  },
  {
    "name": "B.N. Wani",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  },
  {
    "name": "S.R. Bharadwaj",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "R.V. Wandekar",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  },
  {
    "name": "Bina Wani",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  },
  {
    "name": "S.R. Bharadwaj",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

**corresponding**
- AI:    [
  {
    "name": "R.V. Wandekar",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  },
  {
    "name": "B.N. Wani",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  },
  {
    "name": "S.R. Bharadwaj",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "R.V. Wandekar",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  },
  {
    "name": "Bina Wani",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": false
  },
  {
    "name": "S.R. Bharadwaj",
    "rasses": "Chemistry Division, Bhabha Atomic Research Centre, Trombay, Mumbai 400 085, India",
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

## DOI: 10.1016/j.mee.2007.12.032

**rases**
- AI:    [
  {
    "name": "N. Vourdas",
    "rasses": "Microelectronics Research Group, NCSR Demokritos, Institute of Microelectronics, Agia Paraskevi, Athens, 15310, Greece",
    "corresponding_author": false
  },
  {
    "name": "A. Tserepi",
    "rasses": "Microelectronics Research Group, NCSR Demokritos, Institute of Microelectronics, Agia Paraskevi, Athens, 15310, Greece",
    "corresponding_author": false
  },
  {
    "name": "A.G. Boudouvis",
    "rasses": "School of Chemical Engineering, National Technical University of Athens, Athens, Greece",
    "corresponding_author": false
  },
  {
    "name": "E. Gogolides",
    "rasses": "Microelectronics Research Group, NCSR Demokritos, Institute of Microelectronics, Agia Paraskevi, Athens, 15310, Greece",
    "corresponding_author": true
  }
]
- Human: [
  {
    "name": "N. Vourdas",
    "rasses": "Institute of Microelectronics, NCSR Demokritos, Aghia Paraskevi 15310, Greece",
    "corresponding_author": false
  },
  {
    "name": "A. Tserepi",
    "rasses": "Institute of Microelectronics, NCSR Demokritos, Aghia Paraskevi 15310, Greece",
    "corresponding_author": false
  },
  {
    "name": "A.G. Boudouvis",
    "rasses": "School of Chemical Engineering, National Technical University of Athens, Zografou Campus 15780, Greece",
    "corresponding_author": false
  },
  {
    "name": "E. Gogolides",
    "rasses": "Institute of Microelectronics, NCSR Demokritos, Aghia Paraskevi 15310, Greece",
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

## DOI: 10.1016/s0378-1097(99)00346-8

**corresponding**
- AI:    [
  {
    "name": "Guiqing Wang",
    "rasses": "Department of Medical Microbiology, Academic Medical Centre, University of Amsterdam, Meibergdreef 15, 1105 AZ Amsterdam, The Netherlands",
    "corresponding_author": false
  },
  {
    "name": "Alje P. van Dam",
    "rasses": "Department of Medical Microbiology, Academic Medical Centre, University of Amsterdam, Meibergdreef 15, 1105 AZ Amsterdam, The Netherlands",
    "corresponding_author": false
  },
  {
    "name": "Jacob Dankert",
    "rasses": "Department of Medical Microbiology, Academic Medical Centre, University of Amsterdam, Meibergdreef 15, 1105 AZ Amsterdam, The Netherlands",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Guiqing Wang",
    "rasses": "Department of Medical Microbiology, Academic Medical Centre, University of Amsterdam, Meibergdreef 15, 1105 AZ Amsterdam, The Netherlands",
    "corresponding_author": false
  },
  {
    "name": "Alje P. van Dam",
    "rasses": "Department of Medical Microbiology, Academic Medical Centre, University of Amsterdam, Meibergdreef 15, 1105 AZ Amsterdam, The Netherlands",
    "corresponding_author": true
  },
  {
    "name": "Jacob Dankert",
    "rasses": "Department of Medical Microbiology, Academic Medical Centre, University of Amsterdam, Meibergdreef 15, 1105 AZ Amsterdam, The Netherlands",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

## DOI: 10.1038/ng1297-370

**pdf_url**
- AI:    https://www.nature.com/articles/ng1297-370.pdf
- Human: N/A
- landing_page_truth: TBD

## DOI: 10.1039/bk9781782627609-00134

**abstract**
- AI:    There are numerous inheritance patterns involving autosomes and sex chromosomes. This chapter discusses the consequences of possessing, in a pair of homologous chromosomes, a certain combination of alleles. The latter is determined solely by the gender of the animal as far as sex chromosomes are concerned, which obviously cannot be altered by selection. In the case of autosomes, however, this genetic constitution, or genotype, is controlled by both the choice of parents and, to a lesser extent, the random genetic phenomenon of crossover, as discussed in Chapter 3. The chapter introduces basic mating crosses, including monohybrid and dihybrid crosses, and then considers autosomal and sex-linked inheritance patterns in detail. It also examines codominance, incomplete dominance, and the genetic basis of sex determination.
- Human: There are numerous inheritance patterns involving autosomes and sex chromosomes. This chapter discusses the consequences of possessing, in a pair of homologous chromosomes, two mutant alleles or one mutant allele. Dominant or non-dominant (recessive) relationships in autosomes and sex chromosomes are illustrated with a number of human diseases and animal characteristics. Examples discussed include hemophilia in humans and animals, roan cows and palomino horses, cheetahs, lethal alleles, yellow mice, mitochondrial inheritance and Richard III.
- landing_page_truth: TBD

## DOI: 10.1039/c5ra25098f

**pdf_url**
- AI:    https://pubs.rsc.org/en/content/articlepdf/2016/ra/c5ra25098f
- Human: N/A
- landing_page_truth: TBD

## DOI: 10.1063/pt.5.6117

**authors**
- AI:    [
  {
    "name": "Glenn M. Julian",
    "rasses": "",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Shacheenatha Jha",
    "rasses": "",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**rases**
- AI:    [
  {
    "name": "Glenn M. Julian",
    "rasses": "",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Shacheenatha Jha",
    "rasses": "",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**corresponding**
- AI:    [
  {
    "name": "Glenn M. Julian",
    "rasses": "",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Shacheenatha Jha",
    "rasses": "",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

## DOI: 10.1079/cabicompendium.60129

**authors**
- AI:    [
  {
    "name": "CABI",
    "rasses": "CABI Head Office, Wallingford, UK",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Robin Nicholas",
    "rasses": "",
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

**rases**
- AI:    [
  {
    "name": "CABI",
    "rasses": "CABI Head Office, Wallingford, UK",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Robin Nicholas",
    "rasses": "",
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

**corresponding**
- AI:    [
  {
    "name": "CABI",
    "rasses": "CABI Head Office, Wallingford, UK",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Robin Nicholas",
    "rasses": "",
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

## DOI: 10.1086/116973

**pdf_url**
- AI:    https://ui.adsabs.harvard.edu/link_gateway/1994AJ....107.1637B/ADS_PDF
- Human: N/A
- landing_page_truth: TBD

## DOI: 10.1086/ahr/37.2.298

**authors**
- AI:    []
- Human: [
  {
    "name": "J. M. Vincent",
    "rasses": "Pasedena",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**rases**
- AI:    []
- Human: [
  {
    "name": "J. M. Vincent",
    "rasses": "Pasedena",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**corresponding**
- AI:    []
- Human: [
  {
    "name": "J. M. Vincent",
    "rasses": "Pasedena",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

## DOI: 10.1088/0253-6102/36/1/109

**pdf_url**
- AI:    https://iopscience.iop.org/article/10.1088/0253-6102/36/1/109/pdf
- Human: N/A
- landing_page_truth: TBD

## DOI: 10.1088/0256-307x/35/4/045201

**authors**
- AI:    [
  {
    "name": "Chun-Hua Li",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Shao-Wei Wang",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Yun-Hao Liu",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Zhen-Wei Xia",
    "rasses": "School of Information Engineering, North China University of Water Resources and Electric Power, Zhengzhou 450046",
    "corresponding_author": false
  },
  {
    "name": "Xiao-Hui Zhang",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Dan-Dan Zou",
    "rasses": "School of Electrical and Automation Engineering, East China Jiaotong University, Nanchang 330013",
    "corresponding_author": true
  }
]
- Human: [
  {
    "name": "Chun-Hua Li (李春华)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Shao-Wei Wang (王少威)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Yun-Hao Liu (刘云昊)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Zhen-Wei Xia (夏振伟)",
    "rasses": "School of Information Engineering, North China University of Water Resources and Electric Power, Zhengzhou 450046",
    "corresponding_author": false
  },
  {
    "name": "Xiao-Hui Zhang (张小辉)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Dan-Dan Zou (邹丹旦)",
    "rasses": "School of Electrical and Automation Engineering, East China Jiaotong University, Nanchang 330013",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**rases**
- AI:    [
  {
    "name": "Chun-Hua Li",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Shao-Wei Wang",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Yun-Hao Liu",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Zhen-Wei Xia",
    "rasses": "School of Information Engineering, North China University of Water Resources and Electric Power, Zhengzhou 450046",
    "corresponding_author": false
  },
  {
    "name": "Xiao-Hui Zhang",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Dan-Dan Zou",
    "rasses": "School of Electrical and Automation Engineering, East China Jiaotong University, Nanchang 330013",
    "corresponding_author": true
  }
]
- Human: [
  {
    "name": "Chun-Hua Li (李春华)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Shao-Wei Wang (王少威)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Yun-Hao Liu (刘云昊)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Zhen-Wei Xia (夏振伟)",
    "rasses": "School of Information Engineering, North China University of Water Resources and Electric Power, Zhengzhou 450046",
    "corresponding_author": false
  },
  {
    "name": "Xiao-Hui Zhang (张小辉)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Dan-Dan Zou (邹丹旦)",
    "rasses": "School of Electrical and Automation Engineering, East China Jiaotong University, Nanchang 330013",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**corresponding**
- AI:    [
  {
    "name": "Chun-Hua Li",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Shao-Wei Wang",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Yun-Hao Liu",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Zhen-Wei Xia",
    "rasses": "School of Information Engineering, North China University of Water Resources and Electric Power, Zhengzhou 450046",
    "corresponding_author": false
  },
  {
    "name": "Xiao-Hui Zhang",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Dan-Dan Zou",
    "rasses": "School of Electrical and Automation Engineering, East China Jiaotong University, Nanchang 330013",
    "corresponding_author": true
  }
]
- Human: [
  {
    "name": "Chun-Hua Li (李春华)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Shao-Wei Wang (王少威)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Yun-Hao Liu (刘云昊)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Zhen-Wei Xia (夏振伟)",
    "rasses": "School of Information Engineering, North China University of Water Resources and Electric Power, Zhengzhou 450046",
    "corresponding_author": false
  },
  {
    "name": "Xiao-Hui Zhang (张小辉)",
    "rasses": "Department of Information Engineering, Hefei University of Technology, Hefei 230009",
    "corresponding_author": false
  },
  {
    "name": "Dan-Dan Zou (邹丹旦)",
    "rasses": "School of Electrical and Automation Engineering, East China Jiaotong University, Nanchang 330013",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**pdf_url**
- AI:    https://iopscience.iop.org/article/10.1088/0256-307X/35/4/045201/pdf
- Human: N/A
- landing_page_truth: TBD

## DOI: 10.1257/aer.p20171042

**rases**
- AI:    [
  {
    "name": "Susan Athey",
    "rasses": "Stanford U",
    "corresponding_author": false
  },
  {
    "name": "Guido Imbens",
    "rasses": "Stanford U",
    "corresponding_author": false
  },
  {
    "name": "Thai Pham",
    "rasses": "Stanford U",
    "corresponding_author": false
  },
  {
    "name": "Stefan Wager",
    "rasses": "Columbia U and Stanford U",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Susan Athey",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Guido Imbens",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Thai Pham",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Stefan Wager",
    "rasses": "",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

## DOI: 10.1515/9783111535784-008

**abstract**
- AI:    
- Human: N/A`
- landing_page_truth: TBD

## DOI: 10.2307/3283523

**abstract**
- AI:    The tissue cyst stage of Toxoplasma gondii is important in relapsing disease seen in toxoplasmic encephalitis and retinochoroiditis. An in vitro culture system to examine the developmental biology of the tissue cyst stage would greatly aid in our understanding of this stage of the parasite's life cycle. We used transmission electron microscopy (TEM) and acid-pepsin digestion of infected cell cultures to determine th
- Human: N/A
- landing_page_truth: TBD

## DOI: 10.2320/jinstmet1952.61.12_1352

**rases**
- AI:    [
  {
    "name": "高木 真一",
    "rasses": "NKK総合材料技術研究所",
    "corresponding_author": false
  },
  {
    "name": "大内 千秋",
    "rasses": "NKK総合材料技術研究所",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "高木 真一",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "大内 千秋",
    "rasses": "",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

## DOI: 10.3030/821328

**abstract**
- AI:    
- Human: High-efficiency power distribution networks for future regional aircraft
Advanced electrical power distribution networks allowing for less wires throughout the aircraft are crucial for future more-electric aircraft configurations. The EU-funded IDEN project plans to deliver an innovative electrical power distribution network that utilises solid-state transformers. The network will be part of a highly decentralised, modular, flexible smart-grid system. Its enhanced functionalities will enable it to efficiently manage energy, for example reduce or even eliminate the ge
- landing_page_truth: TBD

## DOI: 10.3138/chr-027-04-br24

**abstract**
- AI:    
- Human: High-efficiency power distribution networks for future regional aircraft
Advanced electrical power distribution networks allowing for less wires throughout the aircraft are crucial for future more-electric aircraft configurations. The EU-funded IDEN project plans to deliver an innovative electrical power distribution network that utilises solid-state transformers. The network will be part of a highly decentralised, modular, flexible smart-grid system. Its enhanced functionalities will enable it to efficiently manage energy, for example reduce or even eliminate the generator overload. The new design will become an integral part of the Iron Bird test rig for regional aircraft.
- landing_page_truth: TBD

**pdf_url**
- AI:    https://doi.org/10.3138/chr-027-04-br24
- Human: N/A
- landing_page_truth: TBD

## DOI: 10.3390/polym13183031

**rases**
- AI:    [
  {
    "name": "Vinh Van Tran",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Truong Thi Vu Nu",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Hong-Ryun Jung",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Mincheol Chang",
    "rasses": "",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Vinh Van Tran",
    "rasses": "Alan G. MacDiarmid Energy Research Institute, Chonnam National University, Gwangju 61186, Korea",
    "corresponding_author": false
  },
  {
    "name": "Truong Thi Vu Nu",
    "rasses": "Advanced Institute of Science and Technology, University of Danang, Danang 50000, Vietnam",
    "corresponding_author": false
  },
  {
    "name": "Hong-Ryun Jung",
    "rasses": "Industry-University Cooperation Foundation, Chonnam National University, Gwangju 61186, Korea",
    "corresponding_author": true
  },
  {
    "name": "Mincheol Chang",
    "rasses": [
      "Alan G. MacDiarmid Energy Research Institute, Chonnam National University, Gwangju 61186, Korea",
      "Department of Polymer Engineering, Graduate School, Chonnam National University, Gwangju 61186, Korea",
      "School of Polymer Science and Engineering, Chonnam National University, Gwangju 61186, Korea"
    ],
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

**corresponding**
- AI:    [
  {
    "name": "Vinh Van Tran",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Truong Thi Vu Nu",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Hong-Ryun Jung",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Mincheol Chang",
    "rasses": "",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Vinh Van Tran",
    "rasses": "Alan G. MacDiarmid Energy Research Institute, Chonnam National University, Gwangju 61186, Korea",
    "corresponding_author": false
  },
  {
    "name": "Truong Thi Vu Nu",
    "rasses": "Advanced Institute of Science and Technology, University of Danang, Danang 50000, Vietnam",
    "corresponding_author": false
  },
  {
    "name": "Hong-Ryun Jung",
    "rasses": "Industry-University Cooperation Foundation, Chonnam National University, Gwangju 61186, Korea",
    "corresponding_author": true
  },
  {
    "name": "Mincheol Chang",
    "rasses": [
      "Alan G. MacDiarmid Energy Research Institute, Chonnam National University, Gwangju 61186, Korea",
      "Department of Polymer Engineering, Graduate School, Chonnam National University, Gwangju 61186, Korea",
      "School of Polymer Science and Engineering, Chonnam National University, Gwangju 61186, Korea"
    ],
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

## DOI: 10.3390/su13041644

**rases**
- AI:    [
  {
    "name": "Alpaslan Kelleci",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Oğuz Yıldız",
    "rasses": "",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Alpaslan Kelleci",
    "rasses": "Department of Business Administration, Faculty of Economics, Administrative and Social Sciences, Istanbul Gelisim University, Istanbul 34310, Turkey",
    "corresponding_author": true
  },
  {
    "name": "Oğuz Yıldız",
    "rasses": "Department of Aviation Management, Faculty of Economics, Administrative and Social Sciences, Istanbul Gelisim University, Istanbul 34310, Turkey",
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

**corresponding**
- AI:    [
  {
    "name": "Alpaslan Kelleci",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Oğuz Yıldız",
    "rasses": "",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Alpaslan Kelleci",
    "rasses": "Department of Business Administration, Faculty of Economics, Administrative and Social Sciences, Istanbul Gelisim University, Istanbul 34310, Turkey",
    "corresponding_author": true
  },
  {
    "name": "Oğuz Yıldız",
    "rasses": "Department of Aviation Management, Faculty of Economics, Administrative and Social Sciences, Istanbul Gelisim University, Istanbul 34310, Turkey",
    "corresponding_author": true
  }
]
- landing_page_truth: TBD

## DOI: 10.3724/sp.j.1123.2014.10009

**abstract**
- AI:    建立了离子色谱-抑制型电导检测同时测定食品级润滑油中Cl-、NO3-、SO42- 3种代表性无机阴离子的方法。样品经50%(v/v)甲醇水溶液超声提取,离心分离得到上层水相用0.22 μm混合纤维过滤膜过滤,以15 mmol/L KOH溶液为淋洗液,采用抑制型电导检测器进行检测,外标法定量。在上述条件下,Cl-、NO3-、SO42- 3种无机阴离子在0.10~20.00 mg/L范围内均具有良好的线性关系(R2>0.999);检出限(S/N=3)为0.01~0.03 mg/kg;在1.00、5.00、10.00 mg/kg添加水平下,实际样品中3种阴离子的加标回收率为90.0%~103.6%,相对标准偏差为2.8%~5.7%。结果表明,该方法无需灰化、灰化油相基质等特点的耗时的前处理过程,可以快速、准确实地测定食品级润滑油中Cl-、NO3-、SO42- 3种无机阴离子的含量,适用于润滑油等油品中的痕量无机阴离子的同时分离与测定。
- Human: N/A
- landing_page_truth: TBD

**pdf_url**
- AI:    https://www.chrom-china.com/CN/article/downloadArticleFile.do?attachType=PDF&id=13796
- Human: N/A
- landing_page_truth: TBD

## DOI: 10.4326/jjcvs.28.399

**rases**
- AI:    [
  {
    "name": "服部 良信",
    "rasses": "藤田保健衛生大学胸部外科",
    "corresponding_author": false
  },
  {
    "name": "入山 正",
    "rasses": "藤田保健衛生大学胸部外科",
    "corresponding_author": false
  },
  {
    "name": "渡辺 浩次",
    "rasses": "藤田保健衛生大学短期大学",
    "corresponding_author": false
  },
  {
    "name": "根木 浩路",
    "rasses": "藤田保健衛生大学胸部外科",
    "corresponding_author": false
  },
  {
    "name": "山下 満",
    "rasses": "藤田保健衛生大学胸部外科",
    "corresponding_author": false
  },
  {
    "name": "武田 功",
    "rasses": "藤田保健衛生大学胸部外科",
    "corresponding_author": false
  },
  {
    "name": "杉村 裕志",
    "rasses": "藤田保健衛生大学胸部外科",
    "corresponding_author": false
  },
  {
    "name": "杉村 修一郎",
    "rasses": "藤田保健衛生大学胸部外科",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "服部 良信",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "入山 正",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "渡辺 浩次",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "根木 浩路",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "山下 満",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "武田 功",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "杉村 裕志",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "杉村 修一郎",
    "rasses": "",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

## DOI: 10.53555//kuey.v30i9.5180

**pdf_url**
- AI:    https://kuey.net/index.php/kuey/article/download/5180/5728
- Human: https://kuey.net/index.php/kuey/article/view/5180/5728
- landing_page_truth: TBD

## DOI: 10.5603/ah.2015.0003

**authors**
- AI:    [
  {
    "name": "Marta Sołtysiak",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  },
  {
    "name": "Tomasz Miazgowski",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  },
  {
    "name": "Elżbieta Jaroszyriska",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  },
  {
    "name": "Agnieszka Janowska",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  },
  {
    "name": "Krystyna Widecka",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Marta Sołtysiak",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Tomasz Miazgowski",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Elżbieta Jaroszyńska",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Agnieszka Janowska",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Krystyna Widecka",
    "rasses": "",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

**rases**
- AI:    [
  {
    "name": "Marta Sołtysiak",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  },
  {
    "name": "Tomasz Miazgowski",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  },
  {
    "name": "Elżbieta Jaroszyriska",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  },
  {
    "name": "Agnieszka Janowska",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  },
  {
    "name": "Krystyna Widecka",
    "rasses": "Department of Hypertensiology and Internal Medicine, Pomeranian Medical University in Szczecin",
    "corresponding_author": false
  }
]
- Human: [
  {
    "name": "Marta Sołtysiak",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Tomasz Miazgowski",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Elżbieta Jaroszyńska",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Agnieszka Janowska",
    "rasses": "",
    "corresponding_author": false
  },
  {
    "name": "Krystyna Widecka",
    "rasses": "",
    "corresponding_author": false
  }
]
- landing_page_truth: TBD

## DOI: 10.62480/tjms.2025.vol42.pp71-73

**pdf_url**
- AI:    https://zienjournals.com/index.php/tjms/article/download/6045/4922
- Human: https://zienjournals.com/index.php/tjms/article/download/6045/4922/5916
- landing_page_truth: TBD

## DOI: 10.9734/ajess/2023/v47i31023

**pdf_url**
- AI:    https://journalajess.com/index.php/AJESS/article/download/1023/1998
- Human: https://journalajess.com/index.php/AJESS/article/download/1023/1998/1621
- landing_page_truth: TBD

