...
rases (per-author affiliation — VERBATIM extraction):
  - **Extract the affiliation string EXACTLY as it appears in the page or meta tag. Verbatim. Do not truncate. Do not pick a "primary." Do not strip address details. Do not summarize.**
  - If `citation_author_institution` meta tag for this author has the full string "Department X, University Y, Street Z, City, Postal, Country" — emit that string.
  - If the page joins multiple affiliations on a semicolon ("Inst A, City; Inst B, City") — emit the joined string with the semicolon. (Casey's 2026-04-29 directive: "take the affiliation string exactly as they are and not take portions.")
  - If multiple `citation_author_institution` meta tags exist for the same author (one per affiliation), join them with "; " preserving order.
  - Empty string "" only if NO affiliation is visible anywhere on the page for that author.
  - For static-HTML extraction (Taxicab pipeline): always check `<meta name="citation_author_institution" content="...">` tags first. There is often one meta tag PER affiliation per author — when an author has 3 affils, you'll see 3 tags. Concatenate them with "; ".
  - Examples from gold (note the FULL detail — verbatim with all parts):
    ...
    "Department of Entomology, Montana State University, Bozeman, MT 59717; Small Grain Institute, Private Bag X29, Bethlehem 9700, South Africa"
    "Abteilung für Klinische Toxikologie und Giftnotruf München, Klinikum rechts der Isar der TU München, München, Deutschland"
