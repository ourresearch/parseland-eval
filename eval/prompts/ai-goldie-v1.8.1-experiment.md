...
notes: |
  v1.8.1 is a SCOPE-LIMITED extension of v1.8 that adds ONE thing: non-
  English corresponding-author marker patterns. Per the v1.9 lesson
  (`feedback_prompt_rules_leak.md`), prompt rules that authorize new
  *content sources* leak into adjacent fields. This rule does NOT
  authorize a new content source — it extends an existing field's
  marker list with translations of the markers already there.
  Specifically: the v1.8 prompt already tells the AI to look for
  "Corresponding author:" / "Correspondence to:". v1.8.1 adds the
  Indonesian / Russian / Turkish / Spanish / Portuguese translations
  of those exact phrases.

  This is a strictly safer change than v1.9 because:
    - No new content source is authorized.
    - The matching logic stays "look for marker text near the author".
    - The non-English patterns translate phrases already in the rule.
    - No JSON / no inline-JS / no scope expansion.

  Targeted re-extraction (not a full holdout-50 re-run yet): masharif
  (Indonesian), Luchnikov (Russian — needs cookie-modal handling).
  Score is then verified via merge_livefetch.py + diff_goldie.py.
  Risk if rule still leaks: rolled back, v1.8 stays canonical.
...
corresponding_author (CDL deliverable — strict signals only):
  ...
  - **Non-English marker phrases** (added v1.8.1 — these are translations of
    "Corresponding author" / "Correspondence to", NOT new content sources):
    • Indonesian: "Penulis korespondensi", "Penulis untuk korespondensi"
    • Russian:    "Корреспондирующий автор", "Автор для корреспонденции"
    • Turkish:    "Yazışma adresi", "Yazışma yazarı", "İletişim"
    • Spanish:    "Autor de correspondencia", "Autor para correspondencia"
    • Portuguese: "Autor correspondente", "Autor para correspondência"
    • Chinese:    "通讯作者", "通信作者"
    • Japanese:   "責任著者", "通信著者"
    These markers appear in the same structural positions as the English
    ones (footnote referenced from the author block, or labeled before the
    name). Same matching logic applies — find the author the marker points
    at, set their corresponding_author=true. Do NOT use these markers as a
    source for any other field; do NOT extract new authors / affiliations
    / pdf_urls based on encountering them.
