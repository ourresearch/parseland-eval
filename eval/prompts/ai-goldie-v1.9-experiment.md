...
version: v1.9
date: 2026-05-01
parent: ai-goldie-v1.8.md
notes: |
  v1.9 patches v1.8 against the abstract-extraction failures from the
  holdout-50 v1.8 run (2026-05-01 PM). Three new abstract rules target
  the cached HTML cases where the abstract IS present in the cache but
  the v1.8 selector list missed it. Cases verified by inspecting the
  Taxicab cached HTML directly (eval/scripts/extract_via_taxicab uses
  the same HTML the AI saw):

  - DOI 10.1109/icelmach.2018.8507065 (IEEE Xplore): the full abstract
    is embedded in inline JavaScript as a JSON property:
       "abstract":"This paper presents a study on a novel design..."
    No <meta name="citation_abstract">, no <div class="abstract">.
    Add a fallback: scan inline <script> blocks for a
       \"abstract\":\"<text>\"
    pattern when no abstract has been found. IEEE Xplore + Wiley
    Online Library + a few other React/Next.js publishers use this.

  - DOI 10.25259/nmji_377_2024 (NMJI India, case-report style):
    the page has no separate abstract — the gold "abstract" IS the
    first body paragraph. Body lives inside
       <main><div class="body" lang="en"><p class="" id="-1">...</p>
    For case-report or correspondence articles where no explicit
    abstract block exists, fall back to the first <p> inside
    <main><div class="body"> (or equivalent main-content block).

  - DOI 10.1080/01956051.2025.2517586 (Taylor & Francis): v1.8
    extracted the page's <meta name="description"> tag verbatim,
    which truncates at exactly 200 chars with a literal "...". v1.9
    explicitly bans extracting any string that ends with "..." AND
    is ≤ 250 chars when the article body is reachable elsewhere on
    the page. If the only abstract candidate is a "..."-truncated
    string, return null instead — better empty than wrong.

  Pretty much every other change is carried from v1.8 unchanged.
...
abstract (verbatim only — never paraphrase):
  ...
  2. JSON-LD `description` for ScholarlyArticle (NOT the OpenGraph
     <meta property="og:description"> or generic <meta name="description"> —
     those are SERP truncations, see rule 6 below).
  ...
  5. **Inline-JS JSON fallback** (added v1.9): when the page is built with
     React / Next.js / Angular / inline-data SSR, the abstract often lives
     in a `<script type="application/json">` blob or in a `window.__INITIAL_STATE__`
     payload as `"abstract":"<text>"`. IEEE Xplore is the canonical example:
       grep for the unescaped pattern  `"abstract":"`  inside <script>...</script>
       blocks; the value is the abstract. Only use this fallback when steps 1–4
       returned nothing and the JSON value is > 200 chars (so we don't pick up
       the same truncated meta description).
  6. **Body-paragraph fallback for case-report / correspondence formats**
     (added v1.9): when no explicit abstract block exists at all (NMJI India,
     some BMJ Case Reports, some letters-to-the-editor), the gold abstract IS
     the first body paragraph. Look for the FIRST `<p>` inside
     `<main><div class="body">` or equivalent main-content element. Take the
     full text of that paragraph. If it's < 100 chars, prefer null —
     don't emit a one-sentence stub.
  ...
  ### Anti-truncation rule (added v1.9):
  NEVER emit an abstract that ends with `...` (three dots) AND is ≤ 250
  characters. That shape is a search-result-page meta-description truncation,
  not a real abstract. Examples observed in v1.8 holdout-50:
    "Background and Purpose—We sought to perform a large, prospective,
     multicenter, blinded study comparing power transcranial color duplex
     sonography (power TCDS) with intra-arterial digital subtractio..."
    (200 chars exactly — pulled from <meta name="description">)
  If the only candidate has this shape, look harder via rules 5 and 6
  above. If still nothing, emit null. Better empty than wrong.
