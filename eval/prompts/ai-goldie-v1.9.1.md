...
notes: |
  v1.9.1 is the surgical retry of v1.9-experiment.

  The v1.9 experiment recovered the IEEE Xplore inline-JS abstract case
  but regressed authors / affiliations on the same DOI because the
  inline-JS rule was too unscoped — the AI also picked up authors from
  a `recommendedArticles` JSON sibling.

  v1.9.1 adds ONE targeted rule, gated by DOI prefix:

    For DOIs starting with `10.1109/` (IEEE Xplore), if no abstract is
    found via citation_abstract / JSON-LD / visible-block selectors,
    look for a JSON property `"abstract":"<text>"` in inline <script>
    blocks. The string MUST be > 200 chars. Use ONLY for the abstract
    field — do NOT extract authors, affiliations, pdf_url, or
    corresponding-author from the same JSON blob.

  v1.9.1 explicitly does NOT include:
    - The body-paragraph fallback (the NMJI case is a gold-quality
      issue — gold describes a different paper than the live page).
    - The anti-meta-truncation hard ban (the comparator's truncated-
      meta-prefix-match rule already handles the legitimate cases).
    - Any rule that broadens extraction scope beyond the abstract field.

  Carries forward all v1.8 behavior unchanged.
